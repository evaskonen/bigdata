...
def ingest(report_period=None):
    ...
    # create databas
    database_name = 'STREAMING_CUSTOMER_B'

    # establish Apache Kafka to subscribe events
    topic_name = '1085_top'

    # decrypt data into JSON
    consumer = subscribe_stream(topic_name)

    ts_date = datetime.today().strftime('%Y%m%d-%H%M%S')
    #log_file_name = "Indoor Coordinate Location Dataset" + ".csv"
    #log_file = open(log_file_name, 'w')
    #log_file.write("part_id;ts_date;ts_time;X;Y;Z\n")
    ...
    for msg in consumer:
        ...
        toc = datetime.now()
        message = msg.value
        #log_file.write("INSERTED" + ';'+ str(message['part_id']) + ';'+ str(message['ts_date']) + ';'+ str(message['ts_time']) + ';'+ str(message['X']) + ';' + str(message['Y']) + ';' + str(message['Z']) + '\n')
